...
function Modal({state , setState}) {
    ...
    return (
        <div onClick={closeModal} className={`fixed top-0 w-[100vw] h-[100vh] bg-[#00000099] ${state} items-center justify-center`}>
            <div onClick={(e)=>{e.stopPropagation()}} className="w-[80vw] sm:w-[70vw] lg:w-[40vw] xl:w-[20rem] h-[21rem] rounded-xl bg-box-color p-5 relative">
                <h1 className="text-white font-semibold font-nunito text-sm">Enter the topic to be disscussed</h1>
                <input className="outline-none p-3 caret-white text-white bg-input-box w-[100%] h-6 rounded-md mt-2 mb-4" type="text" />
                ...
                <div className="flex items-center justify-center bg-[#20BD5F] w-36 mx-auto mt-4 rounded-full h-8 space-x-2">
